/**
 * End-to-End Test Script using Puppeteer
 * 
...
async function runTest() {
  ...
  try {
    ...
    await page.type('#new-username', TEST_USERNAME);
    await page.type('#new-password', TEST_PASSWORD);
    await page.type('#confirm-password', TEST_PASSWORD);
    ...
    try {
      await Promise.race([
        page.waitForNavigation({ timeout: 5000 }),
        new Promise((_, reject) => setTimeout(() => reject(new Error('Logout navigation timeout')), 5000))
      ]);
    } catch (e) {
...
(async () => {
  let exitCode = 0;
  console.log('\n🔄 Starting Windsurf Login E2E Test with Puppeteer');
  console.log('================================================');
